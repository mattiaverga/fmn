...
def email(message, recipient):
    ...
    subject_prefix = config.app_conf.get('fmn.email.subject_prefix', u'')
    if subject_prefix:
...
def email_batch(messages, recipient):
    ...
    if recipient.get('verbose', True):
        ...
        email_message = _add_fmn_headers(MIMEMultipart(boundary='=======fmn_email_boundary=='),
                                         recipient=recipient)
        digest = MIMEMultipart(_subtype='digest',
                               boundary='=======next_message_in_digest==')

        email_message['Subject'] = (u'Fedora Notifications Digest '
                                    u'({n} updates)'.format(n=len(messages)))
        ...
        for message_number, message in enumerate(messages, start=1):
            try:
                ...
                _log.exception('fedmsg.meta.msg2subtitle failed to handle %r', message)
                subject = u'Unparsable message subtitle'
...
def _add_fmn_headers(email_message, recipient=None, messages=None):
    """
